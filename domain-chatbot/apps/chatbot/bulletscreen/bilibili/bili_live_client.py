...
class BiliHandler(BaseHandler):
    ...
    async def _on_danmaku(self, client: BLiveClient, message: DanmakuMessage):
        print(message.msg)
        put_chat_message(ChatMessage(
            type="user", user_name=message.uname, content=message.msg, priority=MessagePriority.USER_MESSAGE))
    ...
    async def _on_gift(self, client: BLiveClient, message: GiftMessage):
        ...
        put_chat_message(ChatMessage(
            type="system", user_name=message.uname, content=message_str, priority=MessagePriority.SEND_GIFTS_MESSAGE))
    ...
    async def _on_buy_guard(self, client: BLiveClient, message: GuardBuyMessage):
        ...
        put_chat_message(ChatMessage(
            type="system", user_name=message.gift_name, content=message_str, priority=MessagePriority.SEND_GIFTS_MESSAGE))
    ...
    async def _on_like_click(self, client: BLiveClient, message: LikeInfoV3ClickMessage):
        ...
        put_chat_message(ChatMessage(
            type="system", user_name=message.uname, content=message_str, priority=MessagePriority.TOUCH_THE_HEAD_MESSAGE))
